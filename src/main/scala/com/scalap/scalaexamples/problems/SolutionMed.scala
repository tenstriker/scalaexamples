...
object ArrayProblems {
  ...
  object FindDuplicate {
     ...
     * is a "pigeonhole". Because there are n+1 numbers are n distinct possible numbers, 
    ...
    //Following only work if all integer between 1 to n present in nums, and only two occurance of a duplicate number
    //O(n)
        ...
        if(k == 2) {
          ...
          while(i < sorted.length - k + 1) {
            ...
            if(tmp.nonEmpty){
              ...
              //final list `res` is computed newley within each recursive call
...
object StringProblems {
  ...
  object LengthOfLongestSubstring {
    ...
    def lengthOfLongestSubstring2(s: String) : Int = {
      ...
      while(j < s.length()) {
        ...
        j +=  1
        max = Math.max(max, j - i)
        map.put(ch, j) //reasoning is if we find this ch again we jump i to its next index which is j
      }
      max
    }
  ...
  object LongestPalindrome {
    ...
    def longestPalindrome(s: String): String = {
      ...
      var start, end = 0 //to track eligible start and end index
      var maxlen = 0 //to track maxlen if  needed
